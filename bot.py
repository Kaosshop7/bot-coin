...
class ShopConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await interaction.response.edit_message(content="❌ บอทสิทธิ์ไม่พอให้ยศมึง เอาบอทไว้บนสุดของยศด้วย", embed=None, view=None)
...
async def update_gacha_ui(guild):
    try:
        ...
        if not pool: 
            ...
            for item in pool:
                desc += f"🔸 <@&{item['role_id']}> | โอกาสออก: **{float(item['percent']):.2f}%**\n"
...
async def cmd_edit_rate(interaction: discord.Interaction):
    items = list(gacha_col.find())
    if not items: return await interaction.response.send_message("❌ ตู้กาชายังว่างเปล่า มึงต้องเพิ่มยศก่อน", ephemeral=True)
    await interaction.response.send_message("👇 เลือกยศที่มึงต้องการเปลี่ยนเรท:", view=EditRateView(items, interaction.guild), ephemeral=True)
